...
class CheckoutRepository implements CheckoutRepositoryInterface {
  ...
  Future<Response> placeOrder(
      PlaceOrderBodyModel orderBody, XFile? orderAttachment) async {
  ...
  Future<List<OfflineMethodModel>?> _getOfflineMethodList() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (responseBody is List) {
        // Check if responseBody is a List
        offlineMethodList = [];
        responseBody.forEach((method) =>
            offlineMethodList!.add(OfflineMethodModel.fromJson(method)));
      } else {
        // Handle the case where responseBody is not a List
